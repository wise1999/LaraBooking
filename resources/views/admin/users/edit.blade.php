...
<form action="{{ route('users.update', $user->id) }}" method="POST"
    ...
    @csrf
    @method('PUT')
    <p class="text-lg text-gray-800 font-medium pb-4">Edit User</p>
    <div class="mb-4">
        <label class="block text-sm text-gray-600 mb-2" for="name">User Name</label>
        <input class="w-full px-2 py-1 text-gray-700 bg-gray-200 rounded" id="name" name="name" type="text"
            placeholder="User Name" value="{{ $user->name }}" aria-label="name">
    </div>
    {{--  <div class="mb-4">
